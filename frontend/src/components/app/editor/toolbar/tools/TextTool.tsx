...
export default function TextTool({id} : Props) {
    ...
    return (
        <span className="relative">
            <span className={`transition-all duration-300 flex flex-row gap-1 items-center rounded-full bg-anno-space-700 ${isActiveTool ? 'border-2 bg-zinc-800' : 'border-2 border-transparent'} `}>

                <button onClick={handleClick} type="button" className={`bg-white ${isActiveTool ? 'border-2' : 'border-transparent border hover:bg-gray-200 '} p-1 overflow-hidden rounded-full transition-colors dark:bg-anno-space-700  dark:hover:bg-anno-space-100`}>
