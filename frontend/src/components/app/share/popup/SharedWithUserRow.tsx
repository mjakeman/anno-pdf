    ...
    return (
        <div className={`flex flex-row items-center gap-4 p-2 rounded-xl transition-colors ${showConfirmation && "bg-red-200 dark:bg-anno-space-700"}`}>
            ...
            <div className="flex flex-col grow">
                <p className="font-medium text-black dark:text-white">{fullName} {isCurrentUser && "(you)"} </p>
                <span className="text-slate-500 text-sm dark:text-stone-300">{email}</span>
            </div>

            <button
                className={`${showConfirmation && "hidden"} border-transparent border-2 font-semibold text-md justify-self-end text-anno-red-secondary dark:text-red-500 px-3 py-1 rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-anno-space-100 hover:text-anno-red-primary hover:border-gray-200`}
                type="button"
